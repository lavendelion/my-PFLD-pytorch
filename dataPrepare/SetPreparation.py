...
class ImageDate():
    def __init__(self, line, imgDir, image_size=112):
        ...
        line = line.strip().split()  # 储存一张图的所有点数据+bbox数据+属性数据+图片名称
    ...
    def load_data(self, is_train, repeat, mirror=None):
        # 读取./data/Mirror98.txt文件，并将里面的id序号转换为mirror_idx
        ...
        boxsize = int(np.max(wh)*1.2)  # 人脸bbox的外接正方形，扩大系数1.2
        xy = center - boxsize//2
        x1, y1 = xy
        x2, y2 = xy + boxsize
        ...
        if (dx > 0 or dy > 0 or edx > 0 or edy > 0):
            imgT = cv2.copyMakeBorder(imgT, dy, edy, dx, edx, cv2.BORDER_CONSTANT, 0)  # padding黑边
        ...
        landmark = (self.landmark - xy)/boxsize  # landmark归一化
